...
void System::setSysClock(void)
{
        ...
        **********************************************************************/
        ...
        /* PLLの設定 */
        RCC->PLLCFGR = (16U) | (336U << 6) | (((2U >> 1) - 1) << 16) |
                        (RCC_PLLCFGR_PLLSRC_HSI) | (7U << 24);
...
bool System::canTransmit(CanSelect can_select, uint32_t id, uint8_t *data_p, int data_size, int dead_time, IDmode id_mode)
{
	if(can_select == CAN_1){
		ret1 = can_1.transmit(id,data_p,data_size,dead_time,id_mode);
	}
	else if(can_select == CAN_2){
		ret2 = can_2.transmit(id,data_p,data_size,dead_time,id_mode);
	}
	else{
		return false;
	}
...
void System::get_can_debug(CanSelect can_select, CanDebug *dbg){
	if(can_select == CAN_1){
		can_1.get_can_debug(dbg);
	}
	else if(can_select == CAN_2){
		can_2.get_can_debug(dbg);
	}
}
