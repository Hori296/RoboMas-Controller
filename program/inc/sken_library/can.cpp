...
bool Can::init(Pin tx_pin,Pin rx_pin,CanSelect can_select)
{
	...
	if(can_select == CAN_1){
		...
		filter_config_.FilterScale = CAN_FILTERSCALE_32BIT;
...
void Can::receiveInterruptCallback(void)
{
	if(can_interrupt_flag_){
		...
		(*can_data_).rx_extid = can_rx_.ExtId;
		for(int i=0 ; i < 8 ; i++){
...
HAL_StatusTypeDef Can::transmit(uint32_t id, uint8_t *data_p, int data_size, int dead_time, IDmode id_mode)
{
	if (id_mode == stdid)
	{
	        can_handle_.pTxMsg->StdId = id;
	        can_handle_.pTxMsg->ExtId = 0x00;
	        can_handle_.pTxMsg->IDE = CAN_ID_STD;
	    }
	    else if (id_mode == extid)
	    {
	        can_handle_.pTxMsg->StdId = 0x00;
	        can_handle_.pTxMsg->ExtId = id;
	        can_handle_.pTxMsg->IDE = CAN_ID_EXT;
	    }
...
void Can::get_can_debug(CanDebug *dbg)
{
    uint32_t esr  = can_handle_.Instance->ESR;
    uint32_t msr  = can_handle_.Instance->MSR;
    uint32_t tsr  = can_handle_.Instance->TSR;
    uint32_t rf0r = can_handle_.Instance->RF0R;
    uint32_t rf1r = can_handle_.Instance->RF1R;
    uint32_t err = HAL_CAN_GetError(&can_handle_);
    ...
    dbg->tx_free = 0;
    if (tsr & (1U << 26)) dbg->tx_free++;
    if (tsr & (1U << 27)) dbg->tx_free++;
    if (tsr & (1U << 28)) dbg->tx_free++;

    dbg->fifo0_pending = rf0r & 0x03;
    dbg->fifo1_pending = rf1r & 0x03;
}
